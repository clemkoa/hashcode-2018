...
def solve(data, load, callback, time, **args):
  ...
  with localsolver.LocalSolver() as ls:
    ...
    # Expressions
    def make_lambda(k):
      return lambda i, prev: prev + model.iif(i == 0, model.at(times, 0, cars[k][0] + 1), model.at(times, cars[k][i-1] + 1, cars[k][i] + 1))
    ...
    for c, (late, car) in enumerate(zip(lates, cars)):
      for i in range(N):
        model.constraint(i >= model.count(car) or (late[i] <= model.at(max_lates, car[i])), name=str(c) + '_' + str(i))
